...
class Saga extends EventEmitter {
  ...
  async connect(peer) {
    ...
    console.log(key.toString("hex"), this.db.local.key.toString("hex"));
  ...
  _authorize(key) {
    return new Promise((resolve, reject) => {
      this.db.authorized(key, (err, auth) => {
        ...
        console.log("asdasd", err, auth);
        this.db.authorize(key, (err) => {
          console.log("dsfdfg", err);
          if (err) return reject(err);
          resolve();
        });
      });
  ...
  _updateHistory(onFinish) {
    ...
    const ws = forEachChunk({ objectMode: true }, (data, enc, next) => {
      ...
      if (/operations/.test(key)) {
        console.log(`Check if operation ${key} has been applied`);
    ...
    pump(h, ws, (err) => {
      ...
      values.reverse().forEach((val, idx) => {
        console.log("Applying new operations");
        const key = keys[idx];
        this.emit("operation", { ...val }, key);
        console.log("A new operation has been sent");
